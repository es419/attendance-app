...
function inputDateToDisplay(value: string) {
  ...
  return match ? `${match[3]}.${match[2]}.${match[1]}` : "בחר תאריך";
}
...
export default function Page() {
  ...
  return (
    <main ref={shellRef} className="app-shell">
      ...
      {manualOpen && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setManualOpen(false)}><div className="modal-card manual-modal"><div className="modal-title"><div><p className="eyebrow">משמרת ידנית</p><h2>הוספת שעות</h2></div><button className="icon-button compact" onClick={() => setManualOpen(false)}><Icon name="close"/></button></div><label className="field-label">תאריך</label><div className="native-picker-field"><span className={`native-picker-value ${manualDate ? "" : "is-placeholder"}`}>{inputDateToDisplay(manualDate)}</span><input className="native-picker-input" type="date" value={manualDate} onChange={(e) => setManualDate(e.target.value)} aria-label="תאריך המשמרת"/></div><div className="two-fields"><div><label className="field-label">כניסה</label><div className="native-picker-field"><span className={`native-picker-value ${manualStart ? "" : "is-placeholder"}`}>{manualStart || "בחר שעה"}</span><input className="native-picker-input" type="time" value={manualStart} onChange={(e) => setManualStart(e.target.value)} aria-label="שעת כניסה"/></div></div><div><label className="field-label">יציאה</label><div className="native-picker-field"><span className={`native-picker-value ${manualEnd ? "" : "is-placeholder"}`}>{manualEnd || "בחר שעה"}</span><input className="native-picker-input" type="time" value={manualEnd} onChange={(e) => setManualEnd(e.target.value)} aria-label="שעת יציאה"/></div></div></div><label className="field-label">סה״כ דקות הפסקה</label><input className="text-input ltr-input" type="number" min="0" max="600" value={manualBreakMinutes} onChange={(e) => setManualBreakMinutes(e.target.value)}/><label className="field-label">הערה <span className="optional-label">אופציונלי</span></label><input className="text-input" value={manualNote} onChange={(e) => setManualNote(e.target.value)} placeholder="לדוגמה: עבודה מהבית"/><div className="break-policy-card"><Icon name="coffee"/><div><strong>כלל ההפסקה: {breakAllowanceMinutes} דקות</strong><span>{breakAllowanceMinutes === 0 ? "כל דקות ההפסקה ינוכו מזמן העבודה." : `רק דקות ההפסקה שמעבר ל־${breakAllowanceMinutes} דקות ינוכו מזמן העבודה.`}</span></div></div><button className="primary-action modal-action" disabled={!manualStart || !manualEnd} onClick={() => void addManual()}>{connected && online ? "שמור משמרת" : "שמור מקומית לסנכרון"}</button></div></div>}
