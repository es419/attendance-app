...
import { statusForGoogleError, endBreak } from "@/lib/google-drive";

export async function POST(request: Request) {
  try {
    const body = await request.json();
    if (!body.workspaceId) return NextResponse.json({ error: "חסר קובץ נוכחות" }, { status: 400 });
    const entry = await endBreak(String(body.workspaceId), {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "לא ניתן לחזור מהפסקה" }, { status: statusForGoogleError(error) });
  }
}
